...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    # Zkontrolujeme, zda entry už není nastaven - pokud ano, nejdřív ho unloadujeme
    if entry.entry_id in hass.data.get(DOMAIN, {}):
        _LOGGER.warning(f'Entry {entry.entry_id} is already set up, unloading first')
    ...
    # Kombinovat běžné datapointy s energetickými pro coordinator
    all_datapoints = list(datapoints) if datapoints else []
    if energy_datapoints:
        # Přidat energetické datapointy, pokud ještě nejsou v seznamu
        existing_ids = {dp.get('Id') for dp in all_datapoints}
        for edp in energy_datapoints:
            if edp.get('Id') not in existing_ids:
                all_datapoints.append(edp)
    _LOGGER.info(f"Setting up coordinator with {len(all_datapoints)} datapoints ({len(datapoints) if datapoints else 0} regular, {len(energy_datapoints)} energy)")
    if all_datapoints:
        _LOGGER.debug(f"Datapoint IDs: {[dp.get('Id', 'unknown') for dp in all_datapoints]}")
    coordinator = SiemensOzw672DataUpdateCoordinator(hass, client=client, datapoints=all_datapoints, scaninterval=(timedelta(seconds = conf_scaninterval)))
    await coordinator.async_refresh()
    
    if coordinator.data:
        _LOGGER.info(f"Coordinator returned data for {len(coordinator.data)} datapoints")
        _LOGGER.debug(f"Coordinator data IDs: {list(coordinator.data.keys())}")
    else:
        _LOGGER.warning("Coordinator returned no data!")
    
    if not coordinator.last_update_success:
        _LOGGER.error("Coordinator refresh failed!")
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if entry.entry_id not in hass.data.get(DOMAIN, {}):
        _LOGGER.warning(f'Entry {entry.entry_id} is not set up, nothing to unload')
        return True
    
    coordinator = hass.data[DOMAIN][entry.entry_id]
    # Unloadujeme všechny platformy bez ohledu na coordinator.platforms
    unloaded = all(
