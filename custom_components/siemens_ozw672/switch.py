    ...
    for item in datapoints:
        ...
        for dp_data in entry.data["datapoints"]:
            if dp_data["Id"] == item :
                ...
                prefix_display = ""
                if entry.data.get(CONF_PREFIX_FUNCTION) == True: 
                    prefix_display = prefix_display + f'{dp_data["MenuItem"]} - '
                if entry.data.get(CONF_PREFIX_OPLINE) == True: 
                    prefix_display = prefix_display + f'{dp_data["OpLine"]} '
                dp_config.update({'entity_prefix_display': prefix_display})
                object_id = f"ozw_{slugify(prefix_display + dp_data['Name'])}"
                suggested = f"{SWITCH}.{object_id}"
